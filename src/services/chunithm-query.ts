...
const LXNS_USER_SCORES = 'https://maimai.lxns.net/api/v0/user/chunithm/player/scores'
...
export class ChunithmQuery extends Service {
    ...
    async getB50(session: Session, username?: string): Promise<{
        ...
        if (this.queryConfig.lxnsDevToken) {
            ...
            // 2. LXNS Dev Token - 绑定了好友码时使用
            if (userToken?.lxns_friend_code) {
                const res = await this.queryWithLxnsDevToken(userToken.lxns_friend_code)
                if (res.data) return res
            }

            // 3. LXNS Dev Token - 未绑定 token 的用户通过 QQ 号查询
            if (qq) {
                const res = await this.queryWithLxnsDevTokenByQQ(qq)
                if (res.data) return res
            }
